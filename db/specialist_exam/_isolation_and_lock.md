...
- READ UNCOMMITTED
  - 一番独立性が低く、他のトランザクションの影響を受けやすい。その分、スループットは上がる。
  - 他のトランザクションが正式にコミットする前の変更(UPDATEしたけどコミットしてないなど)を読み込んでしまうので、更新できなかったデータ・ロールバックされたデータでも読み込んでしまう。このことを、**ダーティーリード**という。
- READ COMMITTED
  - 正式にコミットされたデータしか読まないのでダーティーリードは起こらない。
  - 一方で、読み込んだ時の共有ロックは、読み込んだ後に解放してしまう。
  - すると、トランザクション内で最初に読んだ値がトランザクション内の最後の方には他のトランザクションによって書き変わってしまっている可能性がある。
  - 例えば、読み込んだ値に応じて処理を変えたい時などに、途中で値が変わってしまうと誤った処理を実行してしまうことになる。
  - このように、トランザクションの中で読み込んだ値に変化がないとは言い切れない、つまり2度読み込むことができない状態を**ノンリピータブルリード**という。
- REPEATABLE READ
  - 一度データを読み込む際に共有ロックをかけるとトランザクションが終わるまでロックを外さないので、ノンリピータブルリードは起こらない。(ずっと更新できないようになっている)
  - その分、スループットが下がる。
  - 一方で、読み込む既存のデータに対して共有ロックをかけ続けていても新しく追加されるデータに対しては、何もロックをかけない。そのため、トランザクション内でデータが挿入されていてもタイミングによって結果が変わる可能性がある。この状態のことを**ファントムリード**という
- SERIALIZABLE
  - トランザクションが参照したテーブルに対しては、追加、更新、削除の全てを制限することで、最も高い一貫性を担保する方法。
  - 排他待ちが起きやすく、スループットは一番悪い。